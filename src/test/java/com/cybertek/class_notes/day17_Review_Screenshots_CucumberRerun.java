...
public class day17_Review_Screenshots_CucumberRerun {
...
Background:
    -> It is simply applying precondition to scenarios that are in the same feature file.
...
Hooks :
    -> Hooks allows us to add additional pre/post conditions to our scenarios or even our steps.
    -> Before, BeforeStep
    -> After, AfterStep
...
PARAMETERIZATION:
    ...
    -> Not hardcoding: We do not have to go in the implementation(java-selenium) to run the same test on different test data.
    -> Allows us to do easy data driven testing.

    Data driven testing:
        -> running the same test against different data sets.
...
RERUN : IS BUILT IN PLUGIN IN CUCUMBER
    ...
    #2 --> You need to run FailedTestRunner after something has failed.
        -> If your rerun.txt is empty, it means you don't have any tests failing.
